...
def test_materialize_jpeg_calls_vips_thumbnail_and_returns_temp_path(tmp_path: Path) -> None:
    ...
    with patch("rasterize.subprocess.run") as run:
        ...
        dest = materialize_jpeg(tmp_path, avif_rel, max_edge=256, vips_binary="vips")
